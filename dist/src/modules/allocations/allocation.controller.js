...
const allocation_dto_1 = require("./allocation.dto");
const allocationService = __importStar(require("./allocation.service"));
const helpers_1 = require("../../utils/helpers");
async function list(req, res, next) {
    try {
        const { allocations, meta } = await allocationService.listAllocations(req.query);
        (0, helpers_1.sendPaginated)(res, allocations, meta);
    }
    catch (err) {
        next(err);
    }
...
async function create(req, res, next) {
    try {
        const dto = allocation_dto_1.createAllocationSchema.parse(req.body);
        const allocation = await allocationService.createAllocation(req.user.id, dto);
        (0, helpers_1.sendSuccess)(res, allocation, "Allocation created", 201);
    }
    catch (err) {
        next(err);
    }
...
async function updateStatus(req, res, next) {
    try {
        const dto = allocation_dto_1.updateAllocationStatusSchema.parse(req.body);
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
